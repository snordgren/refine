...
sealed abstract class EventAttribute[T] {
  val f: (T => _)
}

sealed abstract class GlobalEvent[T] extends EventAttribute[T] with GlobalAttribute
...
object Attribute {
  ...
  final case class OnAbort(f: UIEvent => _) extends GlobalEvent[UIEvent]
  
  final case class OnActivate(f: UIEvent => _) extends GlobalEvent[UIEvent]
  
  final case class OnBeforeCut(f: DragEvent => _) extends GlobalEvent[DragEvent]
  
  final case class OnBeforeActivate(f: UIEvent => _) extends GlobalEvent[UIEvent]
  
  final case class OnBeforeDeactivate(f: UIEvent => _) extends GlobalEvent[UIEvent]
  
  final case class OnBeforeCopy(f: DragEvent => _) extends GlobalEvent[DragEvent]
  
  final case class OnBeforePaste(f: DragEvent => _) extends GlobalEvent[DragEvent]
  
  final case class OnBlur(f: FocusEvent => _) extends GlobalEvent[FocusEvent]
  
  final case class OnCanPlay(f: Event => _) extends GlobalEvent[Event]
  
  final case class OnCanPlayThrough(f: Event => _) extends GlobalEvent[Event]
  
  final case class OnChange(f: Event => _) extends GlobalEvent[Event]
  
  final case class OnClick(f: MouseEvent => _) extends GlobalEvent[MouseEvent]
  
  final case class OnContextMenu(f: MouseEvent => _) extends GlobalEvent[MouseEvent]
  
  final case class OnCueChange(f: Event => _) extends GlobalEvent[Event]
  
  final case class OnDeactivate(f: UIEvent => _) extends GlobalEvent[UIEvent]
  
  final case class OnDoubleClick(f: MouseEvent => _) extends GlobalEvent[MouseEvent]
  
  final case class OnDrag(f: DragEvent => _) extends GlobalEvent[DragEvent]
  
  final case class OnDragEnd(f: DragEvent => _) extends GlobalEvent[DragEvent]
  
  final case class OnDragEnter(f: DragEvent => _) extends GlobalEvent[DragEvent]
  
  final case class OnDragLeave(f: DragEvent => _) extends GlobalEvent[DragEvent]
  
  final case class OnDragOver(f: DragEvent => _) extends GlobalEvent[DragEvent]
  
  final case class OnDrop(f: DragEvent => _) extends GlobalEvent[DragEvent]
  
  final case class OnEmptied(f: Event => _) extends GlobalEvent[Event]
  
  final case class OnEnded(f: Event => _) extends GlobalEvent[Event]
  
  final case class OnFocus(f: FocusEvent => _) extends GlobalEvent[FocusEvent]
  
  final case class OnFocusIn(f: FocusEvent => _) extends GlobalEvent[FocusEvent]
  
  final case class OnFocusOut(f: FocusEvent => _) extends GlobalEvent[FocusEvent]
  
  final case class OnHelp(f: Event => _) extends GlobalEvent[Event]
  
  final case class OnInput(f: Event => _) extends GlobalEvent[Event]
  
  final case class OnKeyDown(f: KeyboardEvent => _) extends GlobalEvent[KeyboardEvent]
  
  final case class OnKeyPress(f: KeyboardEvent => _) extends GlobalEvent[KeyboardEvent]
  
  final case class OnKeyUp(f: KeyboardEvent => _) extends GlobalEvent[KeyboardEvent]
  
  final case class OnLoad(f: Event => _) extends EventAttribute[Event]
    with ImageAttribute
    with ScriptAttribute
  
  final case class OnLoadedData(f: Event => _) extends GlobalEvent[Event]
  
  final case class OnLoadedMetadata(f: Event => _) extends GlobalEvent[Event]
  
  final case class OnLoadStart(f: Event => _) extends GlobalEvent[Event]
  
  final case class OnMouseDown(f: MouseEvent => _) extends GlobalEvent[MouseEvent]
  
  final case class OnMouseEnter(f: MouseEvent => _) extends GlobalEvent[MouseEvent]
  
  final case class OnMouseLeave(f: MouseEvent => _) extends GlobalEvent[MouseEvent]
  
  final case class OnMouseMove(f: MouseEvent => _) extends GlobalEvent[MouseEvent]
  
  final case class OnMouseOut(f: MouseEvent => _) extends GlobalEvent[MouseEvent]
  
  final case class OnMouseOver(f: MouseEvent => _) extends GlobalEvent[MouseEvent]
  
  final case class OnMouseUp(f: MouseEvent => _) extends GlobalEvent[MouseEvent]
  
  final case class OnMouseWheel(f: WheelEvent => _) extends GlobalEvent[WheelEvent]
  
  final case class OnPause(f: Event => _) extends GlobalEvent[Event]
  
  final case class OnPlay(f: Event => _) extends GlobalEvent[Event]
  
  final case class OnPlaying(f: Event => _) extends GlobalEvent[Event]
  
  final case class OnProgress(f: Any => _) extends GlobalEvent[Event]
  
  final case class OnRateChange(f: Event => _) extends GlobalEvent[Event]
  
  final case class OnReadyStateChange(f: Event => _) extends GlobalEvent[Event]
  
  final case class OnReset(f: Event => _) extends GlobalEvent[Event]
  
  final case class OnScroll(f: UIEvent => _) extends GlobalEvent[UIEvent]
  
  final case class OnSeeked(f: Event => _) extends GlobalEvent[Event]
  
  final case class OnSeeking(f: Event => _) extends GlobalEvent[Event]
  
  final case class OnSelectStart(f: Event => _) extends GlobalEvent[Event]
  
  final case class OnSelect(f: UIEvent => _) extends GlobalEvent[UIEvent]
  
  final case class OnStalled(f: Event => _) extends GlobalEvent[Event]
  
  final case class OnSubmit(f: Event => _) extends GlobalEvent[Event]
  
  final case class OnSuspend(f: Event => _) extends GlobalEvent[Event]
  
  final case class OnTimeUpdate(f: Event => _) extends GlobalEvent[Event]
  
  final case class OnVolumeChange(f: Event => _) extends GlobalEvent[Event]
  
  final case class OnWaiting(f: Event => _) extends GlobalEvent[Event]
